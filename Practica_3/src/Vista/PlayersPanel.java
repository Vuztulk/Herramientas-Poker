...
public class PlayersPanel extends JPanel {
    ...
    public void updatePlayers(List<List<String>> cards, List<String> equities, List<Boolean> activePlayers,
            boolean type_game) {
        for (int player = 0; player < 6; player++) {
            if (cards.size() > player) {
                updatePlayerCards(player, cards.get(player), activePlayers.get(player));
                updatePlayerEquity(player, equities.get(player), activePlayers.get(player), type_game);
            } else {
                updatePlayerCards(player, new ArrayList<>(), false);
                updatePlayerEquity(player, "", false, type_game);
            }
        }
    }
    ...
    private void updatePlayerEquity(int player, String equity, boolean isActive, boolean type_game) {
        if (equityLabels[player] == null) {
            ...
            int x = positions[player][0] - 15;
            int y = positions[player][1] + 110;
